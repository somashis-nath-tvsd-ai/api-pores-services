...
public interface ContentPartnerRepository extends JpaRepository<ExecutionEntity, String>{
    ...
    @Query("DELETE FROM ExecutionEntity c WHERE jsonb_extract_path_text(c.data, 'role') = ?1 AND jsonb_extract_path_text(c.data, 'contentPartnerName') = ?2")
    void deleteByMultipleParams(@Param("role") String role, @Param("contentPartnerName") String contentPartnerName);
    ...
    @Query("SELECT c FROM ExecutionEntity c WHERE jsonb_extract_path_text(c.data, 'role') = ?1 AND jsonb_extract_path_text(c.data, 'link') = ?2")
    List<ExecutionEntity> findByFirstNameAndDepartment(@Param("role")
     String role, @Param("link")
     String link);
